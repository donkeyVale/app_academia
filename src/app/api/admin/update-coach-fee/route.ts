...
export async function POST(req: NextRequest) {
  try {
    ...
    const { currentUserId, userId, academyId, feePerClass } = body as {
    ...
    if (feePerClass == null || Number.isNaN(Number(feePerClass))) {
      return NextResponse.json(
        { error: 'La tarifa por clase es obligatoria y debe ser un número válido.' },
        { status: 400 },
      );
    }

    const normalizedFee = Number(feePerClass);
    if (normalizedFee < 0) {
      return NextResponse.json(
        { error: 'La tarifa por clase no puede ser negativa.' },
        { status: 400 },
      );
    }
    ...
    const { data: existingRow, error: feeSelectErr } = await supabaseAdmin
    ...
    if (feeSelectErr) {
      return NextResponse.json(
        { error: 'No se pudo verificar la tarifa actual del profesor.' },
        { status: 500 },
      );
    }
    ...
    if (existingRow) {
      const { error: updateErr } = await supabaseAdmin
        .from('coach_academy_fees')
        .update({ fee_per_class: normalizedFee })
        .eq('id', existingRow.id);

      if (updateErr) {
        return NextResponse.json(
          { error: 'No se pudo actualizar la tarifa del profesor.' },
          { status: 500 },
        );
      }
    } else {
      const { error: insertErr } = await supabaseAdmin.from('coach_academy_fees').insert({
        coach_id: coachId,
        academy_id: academyId,
        fee_per_class: normalizedFee,
      });

      if (insertErr) {
        return NextResponse.json(
          { error: 'No se pudo guardar la tarifa del profesor.' },
          { status: 500 },
        );
      }
    }
    ...
    return NextResponse.json(
      { error: err?.message ?? 'Error inesperado actualizando tarifa del profesor.' },
      { status: 500 },
    );
  }
}
