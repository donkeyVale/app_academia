...
export async function POST(req: NextRequest) {
  try {
    ...
    const { startIso, endIso } = getUtcRangeForLocalDayUtcMinus3(y, m, d, 0);
    ...
    const bodyText = 'Recordá que tenés clases agendadas, revisá tu agenda!!';
    ...
    console.error('Error en /api/cron/class-reminder-today', e);
    return NextResponse.json({ error: e?.message ?? 'Error ejecutando cron de recordatorio de clase (hoy)' }, { status: 500 });
  }
}
