...
export async function POST(req: NextRequest) {
  try {
    ...
    const { academyId, studentId, studentPlanId, remainingClasses, balance } = body || {};
    ...
    const remainingNum = Number(remainingClasses);
    const balanceNum = Number(balance);

    if (!Number.isFinite(remainingNum)) {
      return NextResponse.json({ error: 'remainingClasses inválido.' }, { status: 400 });
    }
    ...
    const balanceText = `Gs. ${formatGs(balanceNum)}`;
    ...
    console.error('Error en /api/push/balance-reminder', e);
    return NextResponse.json({ error: e?.message ?? 'Error enviando notificación de saldo pendiente' }, { status: 500 });
  }
}
